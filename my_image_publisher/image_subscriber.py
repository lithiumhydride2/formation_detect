...
def image_callback(msg):
    try:
        ...
        # 在掩码中找到红色和绿色物体的轮廓
        _, contours_red_board, _ = cv2.findContours(mask_red_board, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        _, contours_green_board, _ = cv2.findContours(mask_green_board, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        # 确定棋盘的位置（假设棋盘是最大的轮廓）
        x, y, w, h = cv2.boundingRect(cv2.drawContours(mask_all_contours, all_contours, -1, 255, thickness=cv2.FILLED))
        ...
        for i in range(4):
            ...
            for j in range(4):
                ...
                # 计算当前格子中红色和绿色像素数
                red_pixels = np.sum(mask_red_chessboard[y_start:y_end, x_start:x_end] > 0)
                green_pixels = np.sum(mask_green_chessboard[y_start:y_end, x_start:x_end] > 0)
